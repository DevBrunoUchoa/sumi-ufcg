...
A imagem Docker e o ambiente de Compose serão definidos após a consolidação dos comandos de build e inicialização. Essa definição deverá produzir uma única unidade de implantação para o monólito, executar com usuário sem privilégios e limitar a retenção dos logs do contêiner.
