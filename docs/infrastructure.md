...
O código do backend está em `apps/api` (monorepo pnpm workspaces, ver `docs/adr/0001-stack-backend.md`), que já expõe os comandos de build e inicialização. A imagem Docker e o ambiente de Compose serão definidos a partir desse pacote. Essa definição deverá produzir uma única unidade de implantação para o monólito, executar com usuário sem privilégios e limitar a retenção dos logs do contêiner.
